...
var CAT_LABELS_ADMIN = {general:'Top Stories',business:'Business',technology:'Technology',science:'Science',health:'Health',sports:'Sports',entertainment:'Entertainment'};
...
function loadTickerData(){ var r=DB.getTickerHeadlines(); _tickerCache=r.headlines; _allStoppedCache=r.allStopped; }
function getTickerHeadlines(){ if(!_tickerCache) loadTickerData(); return _tickerCache||[]; }
function isTickerAllStopped(){ if(!_tickerCache) loadTickerData(); return _allStoppedCache; }
function setTickerAllStopped(){ var r=DB.toggleAllTicker(); if(r&&typeof r.allStopped!=='undefined') _allStoppedCache=r.allStopped; }
function isArticleStopped(id){ var a=DB.getArticleById(id); return a?!!a.isStopped:false; }
function toggleArticleStopped(id){ DB.toggleStop(id); }
...
function setupSidebarNav() {
  ...
  if (fs) fs.addEventListener('change', function(){ buildAllSubmissions(fs.value); });
}
...
function initTickerManager() {
  renderTickerMasterControl(); renderTickerPreview(); renderTickerHeadlines();
...
function renderTickerMasterControl() {
  var stopped=isTickerAllStopped(), dot=document.getElementById('tickerLiveDot'), label=document.getElementById('tickerMasterLabel'), btn=document.getElementById('tickerMasterBtn');
  if (dot) dot.className='ticker-live-dot'+(stopped?' stopped':'');
  if (label) label.textContent=stopped?'Ticker: Stopped':'Ticker: Live';
  if (btn){ btn.textContent=stopped?'▶ Start All':'⏸ Stop All'; btn.className='btn-ticker-master'+(stopped?' all-stopped':''); }
}
function renderTickerPreview() {
  var track=document.getElementById('tickerPreviewTrack');
  if (!track) return;
  var allStopped=isTickerAllStopped(), headlines=getTickerHeadlines().filter(function(h){ return h.active&&!allStopped; });
  if (headlines.length===0){ track.textContent='— No active headlines —'; track.className='ticker-preview-track paused'; return; }
...
function renderTickerHeadlines() {
  var list=document.getElementById('tickerHeadlineList'), countEl=document.getElementById('tickerCount'), headlines=getTickerHeadlines(), allStop=isTickerAllStopped();
  if (countEl) countEl.textContent=headlines.length;
  if (headlines.length===0){ list.innerHTML='<div class="ticker-empty-msg">No headlines yet. Add one above.</div>'; return; }
  list.innerHTML='';
  headlines.forEach(function(h,idx) {
    var eff=allStop||!h.active, row=document.createElement('div');
    row.className='ticker-headline-row'+(eff?' is-stopped':''); row.id='trow-'+h.id;
    row.innerHTML='<div class="ticker-headline-num">'+(idx+1)+'</div><div class="ticker-headline-body"><div class="ticker-headline-text'+(eff?' stopped-text':'')+'" id="ttext-'+h.id+'">'+escHtml(h.text)+'</div><div class="ticker-headline-status"><span class="status-dot'+(eff?' off':'')+'"></span>'+(eff?'Stopped':'Running on ticker')+'</div></div><div class="ticker-headline-actions"><button class="btn-th-remove" onclick="tickerRemove(\''+h.id+'\')">🗑 Remove</button><button class="btn-th-rewrite" onclick="tickerRewrite(\''+h.id+'\')">✏ Rewrite</button><button class="btn-th-toggle '+(h.active?'is-live':'')+'" id="ttoggle-'+h.id+'" onclick="tickerToggle(\''+h.id+'\')">'+(h.active?'⏹ Stop':'▶ Start')+'</button></div>';
    list.appendChild(row);
  });
}
function addTickerHeadline() {
  var inp=document.getElementById('tickerNewText'), text=inp?inp.value.trim():'';
  if (!text){ showMsg('tickerAddMsg','Please enter a headline.','error'); return; }
  DB.addTickerHeadline(text); _tickerCache=null;
  if (inp) inp.value='';
  showMsg('tickerAddMsg','✅ Headline added!','success');
  renderTickerPreview(); renderTickerHeadlines();
}
function tickerRemove(id) {
  if (!confirm('Remove this headline?')) return;
  DB.deleteTickerHeadline(id); _tickerCache=null;
  renderTickerPreview(); renderTickerHeadlines();
}
function tickerRewrite(id) {
  var headlines=getTickerHeadlines(), h=headlines.find(function(x){ return x.id===id; });
  if (!h) return;
  ...
  rrow.className='ticker-rewrite-row'; rrow.id='trewrite-'+id;
  rrow.innerHTML='<input type="text" class="ticker-rewrite-input" id="trewrite-inp-'+id+'" value="'+escHtml(h.text)+'" maxlength="200" /><button class="btn-rewrite-save" onclick="tickerSaveRewrite(\''+id+'\')">✓ Save</button><button class="btn-rewrite-cancel" onclick="tickerCancelRewrite(\''+id+'\')">✕</button>';
  ...
  if (inp){ inp.focus(); inp.select(); inp.addEventListener('keydown',function(e){ if(e.key==='Enter') tickerSaveRewrite(id); if(e.key==='Escape') tickerCancelRewrite(id); }); }
}
function tickerSaveRewrite(id) {
  var inp=document.getElementById('trewrite-inp-'+id), text=inp?inp.value.trim():'';
  if (!text) return;
  DB.rewriteTickerHeadline(id,text); _tickerCache=null;
  renderTickerPreview(); renderTickerHeadlines();
}
function tickerCancelRewrite(id) {
  var row=document.getElementById('trow-'+id), r=document.getElementById('trewrite-'+id);
  if (row) row.classList.remove('is-rewriting'); if (r) r.remove();
}
function tickerToggle(id) { DB.toggleTickerHeadline(id); _tickerCache=null; renderTickerPreview(); renderTickerHeadlines(); }

/* ARTICLES */
function renderAdminArticles(cat) {
  var list=document.getElementById('adminArticleList');
  if (!list) return;
  var all=DB.getArticles(), filtered=cat==='general'?all:all.filter(function(a){ return a.category===cat; });
...
function confirmDelete(id){ if(confirm('Delete this article?')){ DB.deleteArticle(id); renderAdminArticles(adminCurrentCat); } }

function setupArticleForm() {
  var catSel=document.getElementById('formCategory');
  if (catSel) { CATS.forEach(function(cat){ var opt=document.createElement('option'); opt.value=cat; opt.textContent=CAT_LABELS_ADMIN[cat]; catSel.appendChild(opt); }); }
  var saveBtn=document.getElementById('saveArticleBtn'), cancelBtn=document.getElementById('cancelEditBtn');
  if (saveBtn) saveBtn.addEventListener('click', function() {
    var titleEn=val('formTitleEn'),titleNp=val('formTitle'),summaryEn=val('formSummaryEn'),summaryNp=val('formSummary'),contentEn=val('formContentEn'),contentNp=val('formContent'),author=val('formAuthor'),category=val('formCategory');
    ...
    var data={titleEn:titleEn||titleNp,title:titleNp||titleEn,summaryEn:summaryEn||summaryNp,summary:summaryNp||summaryEn,contentEn:contentEn||contentNp,content:contentNp||contentEn,image:val('formImage'),author,category};
    if (editingId){ DB.updateArticle(editingId,data); showMsg('formMsg',rewriteMode?'Rewritten!':'Updated!','success'); editingId=null; rewriteMode=false; }
    else { DB.addArticle(data); showMsg('formMsg','Published!','success'); }
    clearForm(); resetFormUI(); renderAdminArticles(adminCurrentCat);
  });
  if (cancelBtn) cancelBtn.addEventListener('click', function(){ editingId=null; rewriteMode=false; clearForm(); resetFormUI(); });
}
function resetFormUI() {
  var sb=document.getElementById('saveArticleBtn'),cb=document.getElementById('cancelEditBtn'),bd=document.getElementById('formModeBadge');
  if (sb) sb.textContent='Publish Article'; if (cb) cb.style.display='none';
  if (bd){ bd.textContent='✏️ New Article'; bd.className='form-mode-badge'; }
}
function startEdit(id){ var a=DB.getArticleById(id); if(!a)return; rewriteMode=false; showSection('sectionArticles'); fillForm(a); editingId=id; document.getElementById('saveArticleBtn').textContent='Update Article'; document.getElementById('cancelEditBtn').style.display='inline-block'; var bd=document.getElementById('formModeBadge'); if(bd){bd.textContent='✏️ Editing Article';bd.className='form-mode-badge edit-mode';} document.getElementById('articleForm').scrollIntoView({behavior:'smooth'}); }
function startRewrite(id){ var a=DB.getArticleById(id); if(!a)return; rewriteMode=true; showSection('sectionArticles'); fillForm(a); editingId=id; document.getElementById('saveArticleBtn').textContent='Save Rewrite'; document.getElementById('cancelEditBtn').style.display='inline-block'; var bd=document.getElementById('formModeBadge'); if(bd){bd.textContent='🔄 Rewriting Article';bd.className='form-mode-badge rewrite-mode';} var c=document.getElementById('formContentEn'); if(c){c.focus();c.select();} document.getElementById('articleForm').scrollIntoView({behavior:'smooth'}); }
function fillForm(a){ setVal('formTitleEn',a.titleEn||''); setVal('formTitle',a.title||''); setVal('formSummaryEn',a.summaryEn||''); setVal('formSummary',a.summary||''); setVal('formContentEn',a.contentEn||''); setVal('formContent',a.content||''); setVal('formImage',a.image||''); setVal('formAuthor',a.author||''); setVal('formCategory',a.category||'general'); }
function clearForm(){ ['formTitleEn','formTitle','formSummaryEn','formSummary','formContentEn','formContent','formImage','formAuthor'].forEach(function(id){ setVal(id,''); }); }
...
function buildUserTable() {
  ...
  DB.getUsers().forEach(function(u) {
    html+='<tr><td>'+escHtml(u.name)+'</td><td>'+escHtml(u.email)+'</td><td><span class="role-badge role-'+u.role+'">'+u.role+'</span></td><td>'+(u.joined_at||'—')+'</td><td>'+(u.role!=='admin'?'<button class="btn-delete" onclick="deleteUser(\''+u.id+'\')">Remove</button>':'')+'</td></tr>';
  });
  tbody.innerHTML=html||'<tr><td colspan="5" class="empty-msg" style="text-align:center;padding:20px;">No users found.</td></tr>';
}
...
function buildAllComments() {
  ...
  if (!list) return;
  var all=DB.getAllComments(), articles=DB.getArticles(), html='', hasAny=false;
...
function buildAllSubmissions(filterStatus) {
  var list=document.getElementById('submissionsList'), countEl=document.getElementById('submissionCount');
  if (!list) return;
  filterStatus=filterStatus||'all';
  var submissions=DB.getSubmissions();
  if (filterStatus!=='all') submissions=submissions.filter(function(s){ return s.status===filterStatus; });
  if (countEl) countEl.textContent=submissions.length+' submission'+(submissions.length!==1?'s':'');
  if (submissions.length===0){ list.innerHTML='<p class="empty-msg">No '+(filterStatus!=='all'?filterStatus+' ':'')+'submissions yet.</p>'; return; }
  var html='';
...
function showMsg(id,text,type){ var el=document.getElementById(id); if(!el)return; el.textContent=text; el.className='form-msg '+type; setTimeout(function(){ el.className='form-msg'; },3500); }
