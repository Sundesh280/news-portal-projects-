...
function openArticle(id) {
  ...
  document.getElementById("artCategory").className =
    "cat-badge cat-" + art.category;
  document.getElementById("artTitleEn").textContent =
    art.titleEn || art.title || "";
  document.getElementById("artTitle").textContent = art.title || "";
  document.getElementById("artSummaryEn").textContent =
    art.summaryEn || art.summary || "";
  ...
  var url = encodeURIComponent(
    window.location.href.split("?")[0] + "?article=" + id,
  );
...
function loadComments(artId) {
  ...
  if (comments.length === 0) {
    commentList.innerHTML =
      '<p style="color:#aaa;font-size:0.88rem;">No comments yet.</p>';
    return;
  }
  commentList.innerHTML = comments
    .map(function (c) {
      var isOwner =
        userSession &&
        (userSession.id === c.userId || userSession.role === "admin");
      var actionsHtml = isOwner
        ? '<div class="comment-actions"><button class="btn-edit-comment" onclick="editCommentUI(\'' +
          artId +
          "', '" +
          c.id +
          '\')">Edit</button> <button class="btn-delete-comment" onclick="deleteCommentUI(\'' +
          artId +
          "', '" +
          c.id +
          "')\">Delete</button></div>"
      ...
      return (
        ...
        escHtml(c.date) +
        '</span><p class="comment-text" id="comment-text-' +
        c.id +
        '">' +
        escHtml(c.text) +
        "</p>" +
