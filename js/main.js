...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (dateEl)
    dateEl.textContent = new Date().toLocaleDateString("en-NP", {
      weekday: "long",
      year: "numeric",
      month: "long",
      day: "numeric",
...
function renderArticles(cat) {
  ...
  var heroHtml =
    ...
    featured.image +
    '" alt="" onerror="this.src=\'https://images.unsplash.com/photo-1504711434969-e33886168f5c?w=800&q=80\'" /></div>' +
    '<div class="hero-body">' +
    '<h2 class="hero-title">' +
    ...
    "</p>" +
    '<div class="hero-labels"><span class="hero-badge">TOP STORIES</span><span class="hero-category-pill">' +
  ...
  rest.forEach(function (art) {
    html +=
      ...
      art.image +
      '" alt="" onerror="this.src=\'https://images.unsplash.com/photo-1504711434969-e33886168f5c?w=800&q=60\'" /></div><div class="card-body"><span class="cat-badge cat-' +
      art.category +
      '">' +
      CAT_LABELS[art.category] +
      '</span><h3 class="card-title">' +
      escHtml(art.titleEn) +
      '</h3><p class="card-summary">' +
      escHtml(art.summaryEn) +
      '</p><div class="meta"><span>✍ ' +
      escHtml(art.author) +
      "</span><span>📅 " +
      art.date +
      '</span></div><button class="read-more-btn" onclick="event.stopPropagation();openArticle(\'' +
      art.id +
...
function buildSidebar() {
  var stopped = getStoppedIds();
  var visible = DB.getArticles().filter(function (a) {
    return !stopped.includes(a.id);
  });
  var mostRead = visible
    .slice()
    .sort(function (a, b) {
...
function buildSidebarList(id, articles, showRank) {
  ...
  articles.forEach(function (art, i) {
    html +=
      ...
      '\')"><img src="' +
      art.image +
      '" alt="" onerror="this.src=\'https://images.unsplash.com/photo-1504711434969-e33886168f5c?w=400&q=60\'" />';
    if (showRank) html += '<div class="sidebar-rank">' + (i + 1) + "</div>";
    html +=
      '<div class="sidebar-article-item-body"><div class="sidebar-article-item-title">' +
      escHtml(art.titleEn) +
      '</div><div class="sidebar-article-item-meta">📅 ' +
      art.date +
      " · 👁 " +
      art.views +
      "</div></div></div>";
